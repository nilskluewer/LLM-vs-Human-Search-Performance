...
def evaluate_queries(queries):
    ...
    # Get current date and time to create unique filenames
    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    evaluation_results_file = f'./outputs/evaluation_results_{current_time}.json'
    generated_queries_file = f'./outputs/all_generated_queries_{current_time}.json'
    ...
    evaluation_results.append({
        "query": query,
        "enhanced_search": {
            "results": enhanced_query_search_results,
            "summary": enhanced_query_summarized_answer.content if enhanced_query_summarized_answer else None,
            "duration": enhanced_duration
        },
        "regular_search": {
            "results": regular_query_search_results,
            "summary": regular_query_summarized_answer.content if regular_query_summarized_answer else None,
            "duration": regular_duration
        }
    })
    with open(evaluation_results_file, 'w') as f:
        json.dump(evaluation_results, f, indent=2)
    # Save all generated queries for all searches
    with open(generated_queries_file, 'w') as f:
        json.dump(all_generated_queries, f, indent=2)
    print(f"Results of queries written to {evaluation_results_file}")
    print(f"Generated queries written to {generated_queries_file}")
